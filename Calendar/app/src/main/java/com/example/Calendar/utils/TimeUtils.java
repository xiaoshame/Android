...
public class TimeUtils {
	...
	public static String getDetailLunar(int year,int month,int day) throws Exception{
		String date = TimeUtils.getFormatDate(year, month, day);
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		long time = sdf.parse(date).getTime();
		Lunar lunar = new Lunar(time);
		return lunar.getLunarDateString();
	}
	...
	public static List<DateInfo> initCalendar(String formatDate, int month) throws Exception {
		...
		for (int i = 0; i < allDates.length; i++) {
    		...
    		if (allDates[i] == -1) {
    			...
    			long time = sdf.parse(date).getTime();
    			Lunar lunar = new Lunar(time);
				dateInfo.setDetailLunar(lunar.getLunarDateString());
